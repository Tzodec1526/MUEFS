...
async def test_calculate_fees(db_session):
    ...
    result = await payment_service.calculate_fees(
        db_session, court.id, case_type.id, filing_type="initial"
    )
    assert result.filing_fee_cents == 15000
    assert result.total_cents >= 15000

    # Test motion/subsequent fee is lower and natural
    motion_result = await payment_service.calculate_fees(
        db_session, court.id, case_type.id, filing_type="subsequent"
    )
    assert motion_result.filing_fee_cents == 2000
    assert "motion" in motion_result.fee_description.lower()
